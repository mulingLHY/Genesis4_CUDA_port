...
// cout<<"pack_field_to_soa()"<<endl;
    ...
    std::vector<double> h_re(chunk_cells);
    std::vector<double> h_im(chunk_cells);
    ...
    for (int s = 0; s < fieldSoA->nslice; s += chunk_slices) {
        ...
        genesis4_cuda::copy_host_to_device(h_re.begin(), h_re.begin() + ncopy,
                                           fieldSoA->field_re.begin() + offset);
        genesis4_cuda::copy_host_to_device(h_im.begin(), h_im.begin() + ncopy,
...
// cout<<"unpack_soa_to_field()"<<endl;
    ...
    for (int s = 0; s < fieldSoA->nslice; s += chunk_slices) {
        ...
        genesis4_cuda::copy_device_to_host(fieldSoA->field_re.begin() + offset,
                                           fieldSoA->field_re.begin() + offset + ncopy,
                                           h_re.begin());
        genesis4_cuda::copy_device_to_host(fieldSoA->field_im.begin() + offset,
